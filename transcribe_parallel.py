#!/usr/bin/env python3
"""
faster-whisper を使用した音声文字起こしスクリプト（並列処理版）
Whisper処理とSilero VAD処理を並列で実行します
"""
...
def detect_speech_segments_with_vad(audio_path: str) -> list:
    """Silero VADで音声区間を検出（CPU専用で実行）

    Returns:
        音声区間のリスト [{"start": 開始秒, "end": 終了秒}, ...]
    """
    try:
        print("[VAD] Silero VADで音声区間を検出中...")
...
def transcribe(audio_path: str, model_size: str = "large-v3-turbo", duration_limit: int = None) -> None:
    ...
    # Silero VADを別スレッドで並列実行開始
    print("Silero VADを並列で開始...")
    executor = ThreadPoolExecutor(max_workers=1)
    vad_future: Future = executor.submit(detect_speech_segments_with_vad, audio_path)
    ...
    # ステップ3: Silero VADの結果を取得（並列実行済み）
    print("\nSilero VADの結果を待機中...")
    vad_segments = vad_future.result()
    executor.shutdown(wait=False)
    print(f"VADで検出された音声区間: {len(vad_segments)}個")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("使用方法: python transcribe_parallel.py <音声ファイル> [秒数制限]")
        print("例: python transcribe_parallel.py audio.mp3 180  # 最初の3分だけ")
        sys.exit(1)
